...
const displayMovements = function (account, sort = false) {
    ...
    movs.forEach(function (mov, i) {
        ...
        const movementsDate = formatDate(new Date(account.movementsDates[i]), account.locale);
...
function calcDaysPassed(date1, date2) {
    ...
    return Math.round(Math.abs(date1 - date2) / (SECS_PER_MIN * MINS_PER_HOUR * HOURS_PER_DAY * MILLISECS_PER_SEC));
}

function formatDate(date, locale) {
    const now = new Date();
    const result = calcDaysPassed(date, now);
